...
void ShowMainMenu(ScreenInteractive &screen, GG::ScreenState &state) {
    ...
    // Component renderer = Renderer(menu, [&] {
    //   return vbox({
    //     text("Welcome to GLEEBLEGLOB!") | border | center |
    //     color(Color::CyanLight), menu->Render() | border,
    //   }) | center;
    // });
...
void ShowFunctionMenu(ScreenInteractive &screen, GG::ScreenState &state, int function_selected) {
    std::vector<std::string> functions = {"Glegisterex", "Gloginob",   "Glogoutob",
                                          "Gluploadem",  "Glestreamy", "GLBACKEP"};
    std::vector<std::string> syntax = {"gg register <username>",     "gg login <options> <username>", "gg logout",
    ...
    auto function_name = Renderer([&] {
        return hbox({text("Function: ") | underlined, text(functions[function_selected]) | bgcolor(Color::CyanLight)}) |
               center;
    });
    ...
    auto back_button_renderer = Renderer(back_button, [&] {
        auto style =
            back_button_focused ? (back_button->Render() | inverted | center) : (back_button->Render() | center);
        return style;
    });

    Component table = Container::Vertical({function_name, function_syntax, function_description, back_button_renderer});
...
int GG::Client::_menu(GG::ScreenState state) {
    ...
    while (state != GG::ScreenState::Exit) {
        ...
        case GG::ScreenState::FunctionMenu:
            // This case is handled within ShowDocumentationMenu, so it's not
            // needed here.
            break;
        default:
            break;
