...
GG::Client::Client() {
    ...
    // for tput see
    // https://pubs.opengroup.org/onlinepubs/9699919799.2018edition/utilities/tput.html
...
void GG::Client::_save_session(std::string file_path) {
    ...
    if (!session_file.is_open()) {
        std::cout << "\n--\nError: Couldn't write to session file\n"
                  << "Check write permissions to ~ and ~/.ggsession" << std::endl;
        return;
    }
...
int GG::Client::upload(std::string file_path) {
    ...
    std::string command =
        "ffmpeg -hide_banner -loglevel error -i " + file_path + " -vf \"scale=200:100,fps=8\" " + _bmp_dir + "/%d.bmp";
    ...
    // open connection with server (using api key) and transfer the tarball to
    // the server
...
CharFormat find_closest_color_match(std::unordered_map<uint32_t, CharFormat> &virtual_color_table,
...
void GG::Client::_populate_color_lookup_table(CharFormat *color_lookup_table, ColorSchemeKind kind, size_t num_colors,
                                              bool disable_bg_colors) {
    // for char densities see
    ...
    const float densities[NUM_CHARS] = {0.0,    0.0829, 0.0848, 0.1403, 0.2417, 0.2919,
                                        0.3294, 0.4580, 0.5972, 0.6809, 0.7834, 0.9999};

    // this number converts between a [0,1] range density, and a value that
    // represents the fraction of the viewport that's colored, by division. It's
    // an approximate that I got by measuring the number of unaffected pixels in
    // a character rendered in Consolas.
    ...
    for (size_t fgi = 0; fgi < num_colors; fgi++) {
        ...
        for (size_t bgi = 0; bgi < num_bg_iters; bgi++) {
            ...
            for (size_t chi = 0; chi < NUM_CHARS; chi++) {
                ...
                // 0xf0 converts from 8 bit colors to 4 bit colors (ignore the
                ...
                auto new_format = CharFormat{(uint8_t)fgi, (uint8_t)bgi, (uint8_t)chi};
                if (virtual_color_table.find(combined_rgb) != virtual_color_table.end()) {
                    ...
                    if (kind == ColorSchemeKind::Contrast) {
                        int new_contrast = get_fg_bg_contrast(xterm_colors[new_format.fg_color_index],
                                                              xterm_colors[new_format.bg_color_index]);
                        int existing_contrast = get_fg_bg_contrast(xterm_colors[existing_format.fg_color_index],
                        ...
                        int new_mse = get_fg_bg_mse(xterm_colors[new_format.fg_color_index],
                                                    xterm_colors[new_format.bg_color_index]);
                        int existing_mse = get_fg_bg_mse(xterm_colors[existing_format.fg_color_index],
                                                         xterm_colors[existing_format.bg_color_index]);
    ...
    // We'll have to sort the virtual color table by intensity, then use upper &
    // lower bounds to search a range. Ex: if our closest match has a distance
    // of 16, then all intensity distances larger than 16 can be ignored.
    ...
    if (virtual_color_table.size() < 4096) {
        ...
        for (int r = 0; r < 16; r++) {
            for (int g = 0; g < 16; g++) {
                for (int b = 0; b < 16; b++) {
                    // this is a 24 bit color with only the top 4 MSB of each
                    ...
                    if (virtual_color_table.find(current_rgb) == virtual_color_table.end()) {
                        // if the color doesn't exist in the table, find the
                        // closest existing color & use that instead
                        color_lookup_table[r * 16 * 16 + g * 16 + b] =
                            find_closest_color_match(virtual_color_table, current_rgb);
                    }
                }
            }
        }
...
int GG::Client::stream(std::string token) {
    ...
    while (true) {
        ...
        std::cout << GG::bmp_to_ascii(fname.c_str(), curr_num_cols, curr_num_rows, this->_terminal_supports_color,
                                      color_lookup_table);
        ...
        // debug_log("elapsed_duration: " +
        // std::to_string(elapsed_duration.count())
        // + " ns\n");
